...
import { COMMENTS_COUNT, ModeNavigation, UpdateType } from './const.js';
...
const moviesBlockPresenter = new MoviesBlockPresenter(mainElement, filmsModel, commentsModel, filterModel, apiFilms);
  ...
  .then((films) => {
    ...
    if (films.length !== 0) {
      for (const filmComments of filmsComments) {
        ...
        filmsModel.getFilms()[index].comments.push(filmComments.id);
      }
    }
